...
#nullable disable
public class HungerMultiplier : EffectDefinitionSingle
{
  ...
  private string Description_field = "Hunger";
  ...
  public HungerMultiplier(int ID)
    : base(ID)
  {
    EffectInstance<float, HungerMultiplier> effectInstance = new EffectInstance<float, HungerMultiplier>();
    SerializeFactory.RegisterType<EffectInstance<float, HungerMultiplier>>(effectInstance, effectInstance.Identifier);
  }
  ...
  {
    PeriodicEffectsManager.Apply<float, HungerMultiplier>(this, Entity);
  }

  public override void Remove(Entity Entity, int numberOfStacks)
  {
    PeriodicEffectsManager.Remove<float, HungerMultiplier>(this, numberOfStacks, Entity);
  }
}
