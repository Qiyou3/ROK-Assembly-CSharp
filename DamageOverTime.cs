...
#nullable disable
public class DamageOverTime : EffectDefinitionSingle
{
  private bool Used_field;
  private float Duration_field = 5f;
  private float Value_field = 1f;
  private bool ShowTicker_field = true;
  private ValueDisplayMode DisplayValueAs_field = ValueDisplayMode.Raw;
  private string Description_field = "Health / Sec";
  private string Icon_field = "Happiness_5Rebellious";
  private EffectTickFunc<float> Tick_field = EffectDefinition<float>.DefaultTickFunc;
  private bool EffectCanStack_field = true;
  private bool TimersPerStack_field = true;
  private bool CanExpireWhileOffline_field = true;

  public DamageOverTime(int ID)
    : base(ID)
  {
    EffectInstance<float, DamageOverTime> effectInstance = new EffectInstance<float, DamageOverTime>();
    SerializeFactory.RegisterType<EffectInstance<float, DamageOverTime>>(effectInstance, effectInstance.Identifier);
  }
  ...
  {
    PeriodicEffectsManager.Apply<float, DamageOverTime>(this, Entity);
  }

  public override void Remove(Entity Entity, int numberOfStacks)
  {
    PeriodicEffectsManager.Remove<float, DamageOverTime>(this, numberOfStacks, Entity);
  }
}
